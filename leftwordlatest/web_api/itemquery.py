...
@frappe.whitelist(allow_guest=True)
def item_query(filters=None, order_by=None, start=0, limit=0, search_key="", page=None, products_per_page=None, series_page=True):
    """
    ...
    if filters:
        for key in filters:
            if type(filters.get(key))==list and len(filters.get(key)) > 1:
                custom_filters += " AND WI.{0} IN {1}".format(key, tuple(filters.get(key)))
            elif type(filters.get(key))==list and len(filters.get(key)) == 1:
                custom_filters += " AND WI.{0} ='{1}'".format(key, filters.get(key)[0])
            elif type(filters.get(key))==str:
                custom_filters += " AND WI.{0}='{1}'".format(key, filters.get(key))

    if series_page:
        custom_filters += "AND LENGTH(ITM.custom_series) > 0"
    ...
    items = frappe.db.sql("""
            SELECT 
                ...
                COALESCE(PR_INR.discount_percentage, 0) AS discount_amount_inr,
                COALESCE(PR_USD.discount_percentage, 0) AS discount_amount_usd,
                (CASE WHEN PR_INR.discount_percentage IS NOT NULL THEN 
                    (COALESCE(INR.price_list_rate, 0) - (COALESCE(INR.price_list_rate, 0) * PR_INR.discount_percentage / 100)) 
                ELSE COALESCE(INR.price_list_rate, 0) END) AS discount_percentage_inr,
                (CASE WHEN PR_USD.discount_percentage IS NOT NULL THEN 
                    (COALESCE(USD.price_list_rate, 0) - (COALESCE(USD.price_list_rate, 0) * PR_USD.discount_percentage / 100)) 
                ELSE COALESCE(USD.price_list_rate, 0) END) AS discount_percentage_usd
            FROM 
                `tabWebsite Item` WI
            INNER JOIN
                `tabItem` ITM
            ON
                WI.item_code = ITM.item_code
            LEFT JOIN
                `tabItem Price` INR
            ON
                INR.item_code = ITM.item_code
            AND
                INR.price_list = '{inr_price_list}'
            AND
                INR.currency = "INR"
            LEFT JOIN
                `tabItem Price` USD
            ON
                USD.item_code = ITM.item_code
            AND
                USD.price_list = '{usd_price_list}'
            AND
                USD.currency = "USD"
            LEFT JOIN (
                SELECT
                    parent,
                    GROUP_CONCAT(custom_name SEPARATOR ', ') AS writer_names
                FROM
                    `tabAuthor`
                GROUP BY
                    parent
            ...
            LEFT JOIN
                (
                SELECT 
                    PRB_INR.brand AS rule_brand,
                    PR_INR.discount_percentage,
                    PR_INR.discount_amount
                FROM 
                    `tabPricing Rule` PR_INR
                INNER JOIN
                    `tabPricing Rule Brand` PRB_INR ON PR_INR.name = PRB_INR.parent
                WHERE
                    PR_INR.customer_group = 'Book Club Member'
                    AND PR_INR.currency = 'INR'
                    AND PR_INR.valid_upto >= CURDATE()
                    AND PRB_INR.brand IS NOT NULL
                    AND PR_INR.creation = (
                        SELECT 
                            MAX(PR.creation)
                        FROM
                            `tabPricing Rule` PR
                        INNER JOIN
                            `tabPricing Rule Brand` PRB ON PR.name = PRB.parent
                        WHERE
                            PR.customer_group = 'Book Club Member'
                            AND PR.currency = 'INR'
                            AND PR.valid_upto >= CURDATE()
                            AND PRB.brand IS NOT NULL
                    )
                ) PR_INR ON PR_INR.rule_brand = ITM.brand
            LEFT JOIN
                (
                SELECT 
                    PRB_USD.brand AS rule_brand,
                    PR_USD.discount_percentage,
                    PR_USD.discount_amount
                FROM 
                    `tabPricing Rule` PR_USD
                INNER JOIN
                    `tabPricing Rule Brand` PRB_USD ON PR_USD.name = PRB_USD.parent
                WHERE
                    PR_USD.customer_group = 'Book Club Member'
                    AND PR_USD.currency = 'USD'
                    AND PR_USD.valid_upto >= CURDATE()
                    AND PRB_USD.brand IS NOT NULL
                    AND PR_USD.creation = (
                        SELECT 
                            MAX(PR.creation)
                        FROM
                ...
                ) PR_USD ON PR_USD.rule_brand = ITM.brand                 
            WHERE
                WI.published = 1 AND
                ITM.disabled = False AND
                (COALESCE(INR.selling, 0) = 1 OR COALESCE(USD.selling, 0) = 1)
